...
const roles = [
  ...
  { name: "admin", description: "Has all classroom, student, and user permissions." },
  { name: "student", description: "Has only user permissions." }
];

async function createPermissions(session) {
  for (const permission of permissions) {
    const existingPermission = await Permission.findOne({ name: permission }).session(session);
...
async function createRoles(session) {
  for (const role of roles) {
    const existingRole = await Role.findOne({ name: role.name }).session(session);
    if (!existingRole) {
      const newRole = new Role({ name: role.name, description: role.description });
...
async function addSuperAdmin(session) {
  const superAdminRole = await Role.findOne({ name: "super-admin" }).session(session);
...
async function createRolePermissionMappings(session) {
  ...
  const studentPermissions = ["user:createUser", "user:updateProfile", "user:loginUser"];
  ...
  for (const permissionName of adminPermissions) {
    const permissionDoc = await Permission.findOne({ name: permissionName }).session(session);
    const existingMapping = await RolePermissionMapping.findOne({
